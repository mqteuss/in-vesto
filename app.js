...
window.dismissNotificationGlobal = function (id, btnElement) {
    // Usa o Set em RAM â€” localStorage só é escrito aqui, nunca relido
...
// OTIMIZAÇÃƒO: Intl.NumberFormat instanciado UMA vez e reutilizado.
...
// OTIMIZAÇÃƒO: Set para lookup O(1) em vez de Array.includes() O(97)
...
// Próximo provento â€” 30min no mercado aberto, 4h no fechado
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let watchlist = [];
    // Set em memória para notificações dispensadas â€” populado uma única vez
    ...
    // Range buttons e slider antigos foram removidos â€“ período agora é via bottom sheet

    // â”€â”€â”€ NAVEGAÇÃƒO POR TABS DO MODAL DE DETALHES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    function updateDynamicThemeColor() {
        ...
        // Fora do dashboard â†’ sempre cor padrão
    ...
    function showDetalhesModal(symbol) {
        ...
        // CORREÇÃƒO: Cancela qualquer limparDetalhes pendente do fechamento anterior.
    ...
    function hideDetalhesModal() {
        ...
        // âœ… CORREÇÃƒO: Destruir todas as instâncias ANTES de remover containers do DOM.
        ...
        // âœ… Zerar dimensões do canvas impede que observers residuais encontrem
    ...
    async function carregarTransacoes() {
        ...
        _rebuiltTxIndex(); // Reconstrói índice symbol â†’ transações para getQuantidadeNaData
    }
    ...
    async function processarDividendosPagos() {
        ...
        // 2. OTIMIZAÇÃƒO AQUI: Atualização em Massa no Supabase
    ...
    function calcularCarteira() {
        ...
        // OTIMIZAÇÃƒO: Comparação direta de strings ISO (YYYY-MM-DD) com localeCompare.
    ...
    class VirtualScroller {
        constructor(scrollContainer, listContainer, items, renderRowFn, options = {}) {
            // 1. Inicializa variáveis CRÃTICAS primeiro para evitar crash
                ...
                if (bottom >= startY && pos.top <= endY) {
                    ...
                    if (!this.visibleItems.has(i)) {
                        ...
                        if (pos.item.type === 'header') {
                            ...
                            // Cards precisam de padding lateral (px-4 = 16px, igual Ã  aba Início)
    ...
    function flattenHistoricoData(grupos) {
        ...
        for (const [mes, itens] of Object.entries(grupos)) {
            ...
            // HTML do Header â€” estilo unificado com Mercado + botão de compartilhar
            const headerHtml = `
    ...
    window.renderizarHistoricoGlobal = function () {
        ...
        // OTIMIZAÇÃƒO: localeCompare em strings ISO é equivalente e não cria objetos Date.
        ...
        const rowRenderer = (t) => {
            ...
            // Ãcones condicionais: Casa (ação) ou Prédio (FII)
            ...
            const badge = `<div class="absolute -bottom-0.5 -right-0.5 w-[18px] h-[18px] ${badgeColor} rounded-full flex items-center justify-center border-2 border-black">
    ...
    window.renderizarHistoricoProventosGlobal = function () {
        ...
        const rowRenderer = (p) => {
            ...
            const greenBadge = `<div class="absolute -bottom-0.5 -right-0.5 w-[18px] h-[18px] bg-green-500 rounded-full flex items-center justify-center border-2 border-black">
                <svg class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
            ...
            let label = 'OUTROS';
            if (rawType.includes('JCP')) label = 'JCP';
            else if (rawType.includes('TRIB')) label = 'TRIB';
            else if (rawType.includes('DIV') || rawType.includes('REND')) label = 'DIV';
...
function exibirDetalhesProventos(anoMes, labelAmigavel) {
    ...
    function renderizarGraficoHistorico(dadosExternos = null) {
        ...
        // Destaque do mês atual â€” borda diferente na barra
    ...
    // PATRIMÃ”NIO HISTÓRICO â€” Yahoo Finance v8 (Cálculo On-The-Fly)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     ...
     * Para cada dia, soma: (quantidadeNaData Ã— preço de fechamento) + proventos acumulados.
    ...
    function calcularPatrimonioHistorico(historicoPrecosMap) {
        ...
        // Para cada ativo, cria um Map rápido de date â†’ close
        ...
        // â”€â”€ Pré-calcula proventos pagos ordenados por paymentDate â”€â”€
        ...
        for (const date of sortedDates) {
            // â”€â”€ Acumula proventos recebidos até esta data â”€â”€
    ...
    async function renderizarGraficoPatrimonio(isRetry = false) {
        ...
        // â”€â”€ Busca dados históricos via Yahoo Finance (sempre ALL) â”€â”€
        ...
        // â”€â”€ Filtra por período selecionado â”€â”€
        const hoje = new Date();
        ...
        // â”€â”€ Agrupa por mês (YYYY-MM â†’ último valor do mês) â”€â”€
        const gruposMes = {};
        ...
        // â”€â”€ Calcula movimentações no período â”€â”€
        ...
        // â”€â”€ Calcula proventos recebidos no período â”€â”€
        ...
        // â”€â”€ Função para atualizar stats grid â”€â”€
        ...
        // â”€â”€ Período texto (baseado na primeira transação) â”€â”€
        ...
        function calcularDrawdownMaximo(dataInicio = null, dataFim = null) {
            ...
            // Usa dados DIÃRIOS para capturar quedas intra-mês
            ...
            options: {
                ...
                onClick: (evt, elements) => {
                    if (elements.length > 0) {
                        ...
                        // Usa o último dia DO MÃŠS (não o último dia com dado de preço)
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // GAUGE UNIVERSAL (semicírculo vermelhoâ†’amareloâ†’verde)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    function renderizarTimelinePagamentos() {
        ...
        lista.appendChild(fragment); // único reflow â€” insere tudo de uma vez no DOM real
        container.classList.remove('hidden');
    }
    ...
    let _cacheQtdNaData = {};
    // OTIMIZAÇÃƒO: Ãndice pré-agrupado symbol â†’ transações (reconstruído ao invalidar)
    let _txBySymbol = {};
    ...
    function getQuantidadeNaData(symbol, dataLimiteStr) {
        ...
        // Usa índice pré-agrupado â€” itera apenas transações do symbol (~15x menos)
    ...
    // â”€â”€ Estado de ordenação da carteira â”€â”€
    ...
    async function renderizarCarteira() {
        ...
        // OTIMIZAÇÃƒO: Monta um Map symbol â†’ HTMLElement UMA vez antes do loop
        // para eliminar o querySelector repetido a cada iteração (O(nÂ²) â†’ O(1))
    ...
    function construirQuoteViaYahoo(symbol, historico) {
        ...
        // CORREÇÃƒO: previousClose = fechamento de ONTEM (variação diária correta).
        // chartPreviousClose = fechamento anterior ao início do range do chart (5D),
        // ou seja, ~6 dias úteis atrás â€” NÃƒO serve para variação do dia.
    ...
    async function buscarPrecosCarteira(force = false) {
        ...
        if (!force) {
            for (const ativo of carteiraCalculada) {
                ...
                if (data && !isStale) {
                    staleResults.push(data); // fresh â€” não precisa refetch
    ...
    function processarProventosScraper(proventosScraper = []) {
        ...
        // OTIMIZAÇÃƒO: Map para lookup O(1) em vez de .find() O(n) no loop
    ...
    async function buscarProventosFuturos(force = false) {
        ...
        await Promise.all(ativosParaBuscar.map(async (symbol) => {
            ...
            // CORREÇÃƒO: Verifica se é Array (versão nova) ou Objeto (versão antiga bugada)
    ...
    async function callScraperFundamentosAPI(ticker) {
        // CORREÇÃƒO: cache local para fundamentos (antes não tinha cache nenhum).
    ...
    // â”€â”€ Rankings: Variação do Dia (Altas + Baixas) â”€â”€
            ...
            const renderLista = (containerId, items, tipo) => {
                ...
                el.innerHTML = items.slice(0, 6).map((item) => {
                    ...
                    const iconArrow = isAlta 
                        ? `<svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 8l6 8H6z"/></svg>`
                        : `<svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 16l-6-8h12z"/></svg>`;
                    ...
                    if (logoUrlStr && isAcao(ticker)) {
                        ...
                        iconeHtml = `
                            <div class="w-full h-full flex items-center justify-center bg-[#1a1a1e] rounded-[3px] text-[10px] font-bold text-white tracking-wider absolute inset-0 z-0">${initials}</div>
                        `;
                    }
                    ...
                    return `
                        <div class="py-2.5 px-1 hover:bg-white/[0.015] active:bg-white/[0.03] cursor-pointer transition-colors flex items-center justify-between group border-b border-white/[0.04] last:border-0"
                            ...
                            <div class="flex items-center gap-2.5">
                                <div class="w-[22px] h-[22px] rounded-[3px] flex items-center justify-center flex-shrink-0 relative overflow-hidden bg-white/[0.03]">
                                    ${iconeHtml}
                                </div>
    ...
    async function buscarHistoricoProventosAgregado(force = false) {
        // ALTERAÇÃƒO: Remove o filtro exclusivo de FIIs
    ...
    function verificarNotificacoesFinanceiras() {
        ...
        list.innerHTML = '';
        let count = 0;
        // Usa o Set em RAM â€” sem I/O de localStorage dentro desta função
    ...
    // OTIMIZAÇÃƒO: Debounce genérico â€” agrupa chamadas que chegam em sequência
    ...
    async function atualizarTodosDados(force = false) {
        ...
        try {
            ...
            if (force) {
                // 1. Feedback Tátil (Vibração leve em Android â€” requer gesto do usuário)
        ...
        // OTIMIZAÇÃƒO: usamos renderizarCarteiraDebounced (100 ms) em vez de chamar
        ...
        promessaProventos.then(async proventosFuturos => {
            ...
            // Também via debounce â€” se preços já resolveram, esta chamada será "absorvida"
        ...
        try {
            ...
            // então é seguro desativá-los â€” os cards já têm valores anteriores no DOM.
            ...
            // Radar de notícias â€” só roda aqui porque depende de carteiraCalculada
    ...
    function handleAbrirModalEdicao(id) {
        ...
        // ALTERAÇÃƒO: Aciona o botão correto para animar o toggle
        if (tx.type === 'sell') {
    ...
    //  LÓGICA DO GRÃFICO DE COTAÇÃƒO (VISUAL MELHORADO & SEM CACHE PERSISTENTE)
    ...
    function renderLineChart(dataPoints, range) {
        ...
        // ATUALIZAÇÃƒO DOS VALORES A/V/F (Interativo com o Dedo)
    ...
    function renderCandlestickChart(dataPoints, range) {
        ...
        const updateHeaderStats = (open, close) => {
            ...
            // CORREÇÃƒO: Usando o mesmo estilo compacto e sem o text-xs gigante
        ...
        // â”€â”€â”€ ESCALAS DINÃ‚MICAS (apenas para candles visíveis) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // Converte posição X do canvas â†’ índice global do candle
        ...
        // Converte índice global â†’ posição X no navegador
        ...
        function draw() {
            ...
            // â”€â”€ Linhas de grade Y (preço) â”€â”€
            ...
            // â”€â”€ Linha do último preço visível (tracejada) â”€â”€
    ...
    async function handleMostrarDetalhes(symbol) {
        ...
        // CORREÇÃƒO: Token de sessão que identifica ESTA abertura específica do modal.
        ...
        // â”€â”€â”€ LANÇAR TODAS AS PROMISES EM PARALELO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // Não usar await aqui â€” lançar tudo ao mesmo tempo para que rodem concorrentemente
        ...
        // â”€â”€â”€ FASE 1: Renderiza o modal assim que o PREÇO chegar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if (precoData) {
            ...
            */
            ...
            // â”€â”€ Preço + Variação no cabeçalho fixo â”€â”€
            detalhesPreco.innerHTML = `
        ...
        Promise.all([promiseFundamentos, promiseProvento]).then(([fundData, provData]) => {
            ...
            // Override tipo_ativo do scraper â€” corrige Units (KLBN11, TAEE11) que terminam em 11 mas são Ações
            ...
            */
            ...
            // =========================================================
            // NOVA LÓGICA: COMPARAÇÃƒO (Tabela Horizontal Vesto)
            ...
            if (dados.comparacao && dados.comparacao.length > 0) {
                ...
                if (isStock) {
                    // â”€â”€ AÇÃ•ES: P/L, P/VP, ROE, DY, Val. Mercado, Marg. Líquida â”€â”€
                    ...
                    // â”€â”€ FIIs: DY, P/VP, Patrimônio, Tipo, Segmento â”€â”€
                    let validDy = dados.comparacao.map(i => parseNumberStr(i.dy)).filter(v => v !== null);
                    let validPvp = dados.comparacao.map(i => parseNumberStr(i.pvp)).filter(v => v !== null && v > 0);
            ...
            // â”€â”€ Tab ANÃLISE: Sobre o Ativo + Tabela de Comparação â”€â”€
            const elSobre = document.getElementById('detalhes-sobre-comparacao');
            if (elSobre) {
            ...
            // â”€â”€ Renderiza Gráfico de Rentabilidade (Investidor10) â”€â”€
            ...
            if (rentabilidadeContainer) {
                if (fundamentos.rentabilidade_chart && fundamentos.rentabilidade_chart.profitabilities && fundamentos.rentabilidade_chart.profitabilities.length > 0) {
                    ...
                    // Reset do botão para 2A â€” com delay para o container estar visível
                    requestAnimationFrame(() => {
                        const rentSlider = document.getElementById('rentabilidade-range-slider');
            ...
            // â”€â”€ NOVO: Renderiza Grid de Indicadores Avançados (Apenas Ações) â”€â”€
            ...
            // â”€â”€ NOVO: Renderiza Gráficos de Diversificação de Receita (Apenas Ações) â”€â”€
            ...
            // â”€â”€ NOVO: Gráficos Financeiros (Receitas, Lucro x Cotação, Patrimônio, Payout â€” Apenas Ações) â”€â”€
            ...
            // â”€â”€ Tab PORTFÓLIO: Imóveis (apenas FIIs) â”€â”€
    ...
    function renderizarGraficoProventosDetalhes(rawData) {
        ...
        // ==============================================================
        // HTML DO GRÃFICO DE PROVENTOS (ALINHADO COM COTAÇÃƒO)
        // ==============================================================
    ...
    function getBottomNavPillTarget(index) {
        if (!bottomNavEl || tabButtons.length === 0) return null;
    ...
    function triggerTabButtonBounce(button) {
        if (!button) return;
        button.classList.remove('tab-bounce');
        // Reinicia a animação para permitir bounce em trocas rápidas.
    ...
    function updateBottomNavPillByProgress(progress, options = {}) {
        ...
        const target = getBottomNavPillTarget(index);
        if (!target || !bottomNavPill) return;
    ...
    function mudarAba(tabId, options = {}) {
        ...
        if (slider) {
            if (options.instant) {
                ...
                // Volta para a transição CSS padrão após qualquer drag manual
        ...
        const shouldAnimateTabChange = !options.instant && previousTabIndex !== index;
        updateBottomNavPill(tabId, { instant: !shouldAnimateTabChange });
        if (shouldAnimateTabChange) {
            triggerTabButtonBounce(tabButtons[index]);
        }
    ...
    // â”€â”€ Abas de Categoria (Notícias) â”€â”€
    ...
    // â”€â”€ Busca de Notícias (Debounced) â”€â”€
    if (newsSearchInput) {
    ...
    // â”€â”€ Status do Mercado (Atualização periódica) â”€â”€
    ...
    if (tabDashboard) {
        tabDashboard.addEventListener('click', (e) => {
            ...
            if (toggleCard) {
                // Se clicou dentro do conteúdo expandido (ex: listas ou gráficos), NÃƒO fecha
    ...
    async function carregarDadosIniciais() {
        try {
            ...
            // Leitura única do localStorage â€” a partir daqui todo acesso usa o Set em RAM
            try {
    ...
    async function init() {
        ...
        const iniciarSessaoAutenticada = async (sessionAtual) => {
            verificarStatusPush(); // Fire-and-forget â€” não bloqueia biometria
            ...
            // CRÃTICO: Ativa os skeletons IMEDIATAMENTE após o app ficar visível,
        ...
        // LÓGICA DE SESSÃƒO E ROTEAMENTO
        if (session) {
    ...
    // â”€â”€ Carteira Options Toggle (Menu Retrátil da Busca) â”€â”€
    ...
    listaCarteira.addEventListener('click', e => {
        ...
        // Se NÃƒO estamos em modo lote, o click normal flui para abrir/recolher a gaveta (tratado lá em cima)
        if (!batchEditMode) return;
    ...
    //  LÓGICA DE CÃLCULO DE DY DA CARTEIRA (NOVO RECURSO)
    ...
    // --- FUNÇÃƒO DE BUSCA OTIMIZADA DO IPCA (COM CACHE DE 24H) ---
    ...
    if (proventosPageContent) {
        ...
        proventosPageContent.addEventListener('touchstart', (e) => {
            // CORREÇÃƒO: Se tocar no gráfico, não inicia o arrasto do modal
    ...
    // Lógica Principal de Renderização (VERSÃƒO COM MÉDIA E METAS PERSONALIZÃVEIS)
    ...
    // --- SUPPORT PARA NAVEGAÇÃƒO DE GESTOS (SWIPE BACK) VIA HASH ROUTING ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupDrawer(buttonEl, drawerEl, arrowEl) {
        if (!buttonEl || !drawerEl) return () => false;
        let open = false;
        buttonEl.addEventListener('click', () => {
            open = !open;
            drawerEl.style.maxHeight = open ? (drawerEl.scrollHeight + 500) + 'px' : '0';
    ...
    function calcularJurosCompostos() {
        if (!inPrincipal || !inAporte || !inTaxa || !inAnos || !outInvestido || !outJuros || !outFinal) return;
        ...
        if (isCalcOpen() && drawerCalc) drawerCalc.style.maxHeight = (drawerCalc.scrollHeight + 500) + 'px';
    }
    ...
    const inMetaRendaMensal = document.getElementById('meta-renda-mensal');
    ...
    function calcularMetaRenda() {
        if (!inMetaRendaMensal || !inMetaRendaDy || !outMetaRendaAnual || !outMetaRendaPatrimonio) return;
        const rendaMensal = parseFloat(inMetaRendaMensal.value) || 0;
        const dyAnual = parseFloat(inMetaRendaDy.value) || 0;
        const rendaAnual = rendaMensal * 12;
        const patrimonioNecessario = dyAnual > 0 ? (rendaAnual / (dyAnual / 100)) : 0;

        outMetaRendaAnual.textContent = fmt(rendaAnual);
        outMetaRendaPatrimonio.textContent = fmt(patrimonioNecessario);

        if (isMetaRendaOpen() && drawerMetaRenda) drawerMetaRenda.style.maxHeight = (drawerMetaRenda.scrollHeight + 500) + 'px';
    }
    ...
    const inMetaPrazoObjetivo = document.getElementById('meta-prazo-objetivo');
    const inMetaPrazoInicial = document.getElementById('meta-prazo-inicial');
    const inMetaPrazoTaxa = document.getElementById('meta-prazo-taxa');
    const inMetaPrazoAnos = document.getElementById('meta-prazo-anos');
    ...
    function calcularMetaPrazo() {
        if (!inMetaPrazoObjetivo || !inMetaPrazoInicial || !inMetaPrazoTaxa || !inMetaPrazoAnos || !outMetaPrazoAporte || !outMetaPrazoTotalAportes || !outMetaPrazoObjetivoFinal) return;
        const objetivo = parseFloat(inMetaPrazoObjetivo.value) || 0;
        const inicial = parseFloat(inMetaPrazoInicial.value) || 0;
        const taxaAnual = parseFloat(inMetaPrazoTaxa.value) || 0;
        const anos = parseFloat(inMetaPrazoAnos.value) || 0;
        const n = Math.max(0, anos * 12);

        let aporteMensal = 0;
        if (n > 0 && objetivo > 0) {
            if (taxaAnual === 0) {
                aporteMensal = Math.max(0, (objetivo - inicial) / n);
            } else {
                const tm = Math.pow(1 + taxaAnual / 100, 1 / 12) - 1;
                const fvInicial = inicial * Math.pow(1 + tm, n);
                const fatorSerie = (Math.pow(1 + tm, n) - 1) / tm;
                aporteMensal = fatorSerie > 0 ? Math.max(0, (objetivo - fvInicial) / fatorSerie) : 0;
            }
        }

        const totalAportes = aporteMensal * n;
        outMetaPrazoAporte.textContent = fmt(aporteMensal);
        outMetaPrazoTotalAportes.textContent = fmt(totalAportes);
        outMetaPrazoObjetivoFinal.textContent = fmt(objetivo);

        if (isMetaPrazoOpen() && drawerMetaPrazo) drawerMetaPrazo.style.maxHeight = (drawerMetaPrazo.scrollHeight + 500) + 'px';
    }
    [inMetaPrazoObjetivo, inMetaPrazoInicial, inMetaPrazoTaxa, inMetaPrazoAnos].forEach(el => el?.addEventListener('input', calcularMetaPrazo));
    calcularMetaPrazo();
});

// â”€â”€â”€ Perfil de Usuário â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Self-contained module â€” não depende de vestoDB nem showToast (ambos estão
