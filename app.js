...
function criarCardElemento(ativo, dados) {
    ...
    // 1. Definição de Cores do Avatar baseado no Ticker (Hash simples)
    // Isso garante que MXRF11 seja sempre da mesma cor, etc.
    const ticker = ativo.symbol;
    const colors = [
        'bg-purple-600', 'bg-blue-600', 'bg-emerald-600', 
        'bg-orange-600', 'bg-pink-600', 'bg-indigo-600', 'bg-cyan-600'
    ];
    // Soma os códigos char do ticker para escolher uma cor consistente
    const charSum = ticker.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
    const avatarBg = colors[charSum % colors.length];
    
    // Iniciais (ex: MXRF11 -> MX)
    const initials = ticker.substring(0, 2);

    // 2. Lógica de L/P (Badge e Texto)
    // Se lucro: Verde / Se prejuízo: Vermelho
    const isLucro = lucroPrejuizo >= 0;
    const corLPTexto = isLucro ? 'text-green-500' : 'text-red-500';
    const setaLP = isLucro ? '▲' : '▼';

    // 3. HTML do Provento (Reaproveitado, mas simplificado para o drawer)
    let proventoHtml = '';
    ...
    // Removemos 'card-bg' e 'rounded', usamos o estilo de lista
    card.className = 'group'; // Wrapper para controle
    card.setAttribute('data-symbol', ativo.symbol);

    card.innerHTML = `
        <div class="wallet-item" data-symbol="${ativo.symbol}" data-action="toggle">
            
            <div class="flex items-center min-w-0">
                <div class="asset-avatar ${avatarBg} shadow-lg shadow-${avatarBg.replace('bg-', '')}/20">
                    ${initials}
                </div>
                <div class="min-w-0 flex flex-col">
                    <h3 class="text-base font-bold text-white leading-tight truncate">${ativo.symbol}</h3>
                    <span class="text-xs text-gray-500 font-medium" data-field="cota-qtd">${ativo.quantity} cotas</span>
                </div>
            </div>
            
            <div class="flex items-center gap-3">
                <div class="flex flex-col items-end">
                    <span data-field="posicao-valor" class="wallet-value-primary">${dadoPreco ? formatBRL(totalPosicao) : '...'}</span>
                    
                    <div class="flex items-center gap-1">
                        <span data-field="preco-valor" class="text-[10px] text-gray-500">${precoFormatado}</span>
                        <span data-field="pl-valor" class="text-[10px] font-bold ${corLPTexto} bg-gray-800/50 px-1.5 py-0.5 rounded ml-1">
        ...
        <div id="drawer-${ativo.symbol}" class="card-drawer border-b border-[#1C1C1E]">
            <div class="drawer-content px-4 pb-4 pt-1">
                
                <div class="grid grid-cols-2 gap-4 mt-2">
                    <div class="p-2 rounded-lg bg-[#1C1C1E] border border-[#2C2C2E]">
                        <span class="text-[10px] text-gray-500 block uppercase">Preço Médio</span>
                        <span class="text-sm font-semibold text-white">${formatBRL(ativo.precoMedio)}</span>
                    </div>
                    <div class="p-2 rounded-lg bg-[#1C1C1E] border border-[#2C2C2E]">
                        <span class="text-[10px] text-gray-500 block uppercase">Custo Total</span>
                        <span data-field="custo-valor" class="text-sm font-semibold text-white">${formatBRL(custoTotal)}</span>
                    </div>
                    <div class="p-2 rounded-lg bg-[#1C1C1E] border border-[#2C2C2E]">
                        <span class="text-[10px] text-gray-500 block uppercase">Lucro/Prej. (R$)</span>
                        <span class="text-sm font-semibold ${corLPTexto}">${formatBRL(lucroPrejuizo)}</span>
                    </div>
                    <div class="p-2 rounded-lg bg-[#1C1C1E] border border-[#2C2C2E]">
                        <span class="text-[10px] text-gray-500 block uppercase">Var. Dia</span>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button class="flex-1 py-2 px-4 text-xs font-bold text-gray-200 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors border border-gray-700" data-symbol="${ativo.symbol}" data-action="details">
                        Ver Detalhes
                    </button>
                    <button class="py-2 px-4 text-xs font-bold text-red-400 bg-red-900/10 hover:bg-red-900/30 border border-red-900/30 rounded-lg transition-colors" data-symbol="${ativo.symbol}" data-action="remove">
                        Remover
                    </button>
                </div>
            </div>
