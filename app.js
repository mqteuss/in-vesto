...
// ======================================================
// 1. FUNÇÃO QUE ABRE O MODAL (A que faltava)
// ======================================================
window.openPagamentosModal = function(listaPagamentos) {
    const modal = document.getElementById('pagamentos-page-modal');
    const content = document.getElementById('tab-pagamentos-content');

    if (modal && content) {
        // A. Renderiza o conteúdo novo (Agrupado por Mês)
        renderizarListaPagamentosModal(listaPagamentos);

        // B. Prepara a animação
        modal.classList.remove('hidden');
        
        // Truque do navegador: Força ele a "ler" o elemento antes de animar
        void modal.offsetWidth; 

        // C. Animação de Entrada
        modal.classList.add('visible'); // Escurece o fundo
        content.style.transform = 'translateY(0)'; // Sobe o painel
        
        // D. Trava o scroll da página de trás
        document.body.style.overflow = 'hidden';
    }
}

// ======================================================
// 2. FUNÇÃO QUE DESENHA A LISTA (Visual Polido)
// ======================================================
function renderizarListaPagamentosModal(listaPagamentos) {
    ...
    if (!listaPagamentos || listaPagamentos.length === 0) {
        container.innerHTML = `
            <div class="flex flex-col items-center justify-center h-64 opacity-50">
                <p class="text-sm font-medium text-gray-400">Nenhum pagamento futuro.</p>
            </div>`;
        return;
    }

    // Ordena por Data
    const sorted = [...listaPagamentos].sort((a, b) => new Date(a.paymentDate) - new Date(b.paymentDate));

    // Agrupa por Mês/Ano
    const grupos = {};
    sorted.forEach(pgto => {
        const dataKey = pgto.paymentDate.substring(0, 7); // "2025-10"
        if (!grupos[dataKey]) {
            grupos[dataKey] = { items: [], total: 0, dataRef: new Date(pgto.paymentDate) };
        }
        
        // Calcula total (Valor * Qtd)
        const ativo = carteiraCalculada.find(c => c.symbol === pgto.symbol);
        const qtd = ativo ? ativo.quantity : 0;
        const totalReceber = pgto.value * qtd;

        grupos[dataKey].items.push({ ...pgto, totalReceber });
        grupos[dataKey].total += totalReceber;
    });

    // Renderiza cada grupo
    Object.keys(grupos).forEach(key => {
        const grupo = grupos[key];
        const mesNome = grupo.dataRef.toLocaleString('pt-BR', { month: 'long', year: 'numeric' });
        const totalGrupoFmt = grupo.total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

        // Cabeçalho do Mês
        const headerHTML = `
            <div class="payment-group-header">
                <span class="group-month-name">${mesNome}</span>
                <span class="group-month-total">Total: ${totalGrupoFmt}</span>
            </div>
        `;
        ...
        grupo.items.forEach(item => {
            ...
            const dia = parts[2];
            const dataObj = new Date(parts[0], parts[1] - 1, parts[2]);
            const diaSemana = dataObj.toLocaleString('pt-BR', { weekday: 'short' }).replace('.', '');
            
            // Verifica Hoje
            const hoje = new Date(); hoje.setHours(0,0,0,0);
            const dataItem = new Date(dataObj); dataItem.setHours(0,0,0,0);
            const isHoje = dataItem.getTime() === hoje.getTime();

            const classeCor = isHoje ? 'text-today' : 'text-future';
            const textoStatus = isHoje ? 'Cai Hoje' : 'Agendado';
            const classeBox = isHoje ? 'bg-today' : '';
            const valorFmt = item.totalReceber.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

            const rowHTML = `
                <div class="payment-row-card" onclick="if(typeof abrirDetalhesAtivo === 'function') abrirDetalhesAtivo('${item.symbol}')">
                    <div class="flex items-center">
                        <div class="pay-date-box ${classeBox}">
                            <span class="pay-day-number">${dia}</span>
                            <span class="pay-day-week">${diaSemana}</span>
                        </div>
                        <div class="pay-info">
                            <span class="pay-ticker">${item.symbol}</span>
                            <span class="pay-type">Provento</span>
                        </div>
                    </div>
                    <div class="flex flex-col items-end">
                        <span class="pay-value ${classeCor}">+${valorFmt}</span>
                        <span class="pay-status ${classeCor} opacity-70">${textoStatus}</span>
                    </div>
                </div>
            `;
    ...
    container.insertAdjacentHTML('beforeend', '<div class="h-12"></div>');
}
