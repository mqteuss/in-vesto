...
const isFII = (symbol) => {
    ...
    // Geralmente FIIs terminam em 11 (mas algumas units de ações também, ex: TAEE11)
    // Uma heurística melhor: Se não estiver numa lista de exceções conhecidas de Units
    const unitsAcoes = ['ALUP11', 'TAEE11', 'KLBN11', 'SAPR11', 'SANB11', 'TIET11']; 
...
async function handleMostrarDetalhes(symbol) {
    ...
    // --- 3. BUSCA DE FUNDAMENTOS (NOSSO SCRAPER) ---
    let fundamentos = {};
    let nextProventoData = null;
    const eFundoImobiliario = isFII(symbol);

    // Renderiza gráfico histórico apenas se for FII (por enquanto)
    if (eFundoImobiliario) {
    ...
    try {
        ...
        const promises = [callScraperFundamentosAPI(symbol)];
        // Só busca próximo provento detalhado se for FII (para ações o scraper de fundamentos já pode trazer algo ou ser expandido depois)
        if (eFundoImobiliario) {
            promises.push(callScraperProximoProventoAPI(symbol));
        }

        const results = await Promise.all(promises);
        fundamentos = results[0] || {};
        if (eFundoImobiliario) nextProventoData = results[1];

    } catch (e) { 
        console.error("Erro dados extras", e); 
    }
    ...
    if (precoData) {
        ...
        // Card: Próximo Provento (Se houver)
        ...
        const dados = { 
            ...
            divida: fundamentos.divida_liquida_ebitda || '-'
        };
        ...
        // === RENDERIZAÇÃO CONDICIONAL (AÇÕES vs FIIs) ===
        let cardsDestaqueHtml = '';
        ...
        if (eFundoImobiliario) {
            ...
            listaDetalhesHtml = `
                <div class="w-full bg-black border border-[#2C2C2E] rounded-2xl overflow-hidden px-4">
                    ...
                    ${renderRow('Margem Líq.', dados.margem_liquida)}
                    ${renderRow('Dív.Líq/EBITDA', dados.divida)}
                    <div class="flex justify-between items-center py-3.5">
                        <span class="text-sm text-[#888888] font-medium">Var. 12 Meses</span>
                        <span class="text-sm font-bold ${corVar12m} text-right flex items-center gap-1">
                            ${icon12m} ${dados.variacao_12m}
        ...
        detalhesPreco.innerHTML = `
            <div class="col-span-12 w-full flex flex-col gap-3">
                ...
                <div class="w-full bg-black border border-[#2C2C2E] rounded-2xl px-4 pt-2 pb-2">
                    ${renderRow('Segmento', fundamentos.segmento || '-')}
                    ${renderRow('Tipo', fundamentos.tipo_fundo || fundamentos.tipo_gestao || '-')}
                    ${renderRow('CNPJ', fundamentos.cnpj || '-')}
                    <div class="flex justify-between items-center py-3.5">
                        <span class="text-sm text-[#888888] font-medium">Cotistas</span>
                        <span class="text-sm font-semibold text-[#e5e5e5] text-right">${fundamentos.num_cotistas || '-'}</span>
                    </div>
                </div>
