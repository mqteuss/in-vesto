...
async function scrapeFundamentos(ticker) {
    try {
        ...
        // Buscar dados estruturados via scanner paralelo (AeroScrape v2: seletores compostos + extract)
        const [resCards, resCells, resTable, resAbout, resLogo, resCompareUrl, resProps, resPropsSmall] = await Promise.all([
            ...
            aeroScrape(urlToUse, { selector: '.header-company img, #header-container img, .brand-company img', extract: 'src' }).catch(() => ({ data: [] })),
            aeroScrape(urlToUse, { selector: '#table-compare-fiis, #table-compare-segments', extract: 'data-url' }).catch(() => ({ data: [] })),
            aeroScrape(urlToUse, { selector: 'div.card-propertie h3' }).catch(() => ({ data: [] })),
            aeroScrape(urlToUse, { selector: 'div.card-propertie small' }).catch(() => ({ data: [] }))
        ]);
        ...
        // Imóveis do FII via AeroScrape v2 (seletores descendentes compostos)
        if (resProps && resProps.data && resProps.data.length > 0) {
            const smalls = (resPropsSmall && resPropsSmall.data) ? resPropsSmall.data : [];
            let smallIdx = 0;
            resProps.data.forEach(nome => {
                if (!nome) return;
                let estado = '', abl = '';
                // Cada card-propertie tem ~2 smalls (Estado + ABL)
                for (let s = 0; s < 2 && smallIdx < smalls.length; s++, smallIdx++) {
                    const t = smalls[smallIdx] || '';
        ...
        // Logo via AeroScrape v2 (extract: 'src')
        if (resLogo && resLogo.data && resLogo.data.length > 0) {
            dados.logo_url = resLogo.data[0];
        ...
        // AeroScrape v2: data-url já extraído via extract: 'data-url' (sem regex)
        const apiUrl = (resCompareUrl && resCompareUrl.data && resCompareUrl.data.length > 0) ? resCompareUrl.data[0] : null;
...
async function scrapeRankings() {
    ...
    try {
        // AeroScrape v2: extract múltiplo — pega href + texto de cada link em 1 request
        ...
        // Separar altas/baixas via blocos do HTML
